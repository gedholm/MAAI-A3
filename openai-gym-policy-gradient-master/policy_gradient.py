...
class PolicyGradient:
    ...
    def choose_action(self, observation):
        ...
        prob_raveled = prob_weights.ravel()
        # Select action using a biased sample
        # this will return the index of the action we've sampled
        #action = np.random.choice(range(len(prob_weights.ravel())), p=prob_raveled)
        action = np.argmax(prob_raveled)
        return action
    ...
    def discount_and_norm_rewards(self):
        ...
        discounted_episode_rewards /= (np.std(discounted_episode_rewards) + 10**(-7))
    ...
    def run_simulation(self, max_frames, env, render):
        ...
        while not done:
            ...
            self.store_transition(observation, a, reward)
