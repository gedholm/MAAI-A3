...
def makeAndtrainNetwork():
    ...
    loss = -tf.reduce_mean(tf.math.multiply( weights_ph , log_probs))
    ...
    def train_one_epoch():
        ...
        for j in range(1):
            ...
            while True:
                ...
                if (not finished_rendering_this_epoch) and render and (np.mod(i,5) == 0) and j == 0:
                    env.render()
                ...
                if done or (iter > iterMax):
                    ...
                    for r in range(ep_len):
                        cummulative = 0
                        for s in range(20):
                            cummulative += ep_rews[np.minimum(i+s,ep_len-1)]*np.power(0.9,s)
